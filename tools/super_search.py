...
DEFAULT_MCPORTER_CONFIG = PROJECT_ROOT / "config" / "mcporter.json"
...
def _jina_failed(text: str) -> bool:
    """判断 Jina Reader 是否失败/被限流（空、错误 JSON、或拿不到正文）。"""
    if not text or not text.strip():
        return True
    if "CompromiseError" in text or '"data":null' in text:
        return True
    stripped = text.strip()
    if stripped.startswith("{") and "Markdown Content:" not in text and "Title:" not in text:
        return True
    return False


def _camoufox_read(url: str) -> "SearchResult | None":
    """Jina 被限流时用 camoufox 无头读正文兜底（复用 cam_fetch.py）。
    可移植：依次找 同级engines/ → 同目录 → ~/video-factory 里的 cam_fetch.py。"""
    import os
    here = os.path.dirname(os.path.abspath(__file__))
    for d in (os.path.join(here, "..", "engines"), here, os.path.expanduser("~/video-factory")):
        d = os.path.abspath(d)
        if os.path.exists(os.path.join(d, "cam_fetch.py")) and d not in sys.path:
            sys.path.insert(0, d)
    try:
        from cam_fetch import fetch_url_text
        title, author, body = fetch_url_text(url)
    except Exception:
        return None
    if not body:
        return None
    return SearchResult(
        ...
        title=title or url,
        url=url,
        snippet=compact_text(body, 900),
        meta={"author": author, "fallback": "jina->camoufox"},
    )
...
class GitHubProvider:
    ...
    def _run(self, query: str, limit: int) -> list[dict[str, Any]]:
        argv = [
            "gh",
            "search",
            "repos",
    ...
    def search(self, query: str, limit: int = 5) -> list[SearchResult]:
        # gh search repos 把多个词按 AND 处理：自然语言长 query 常匹配 0 仓库、
        # 静默返回 []。空结果时逐步回退到更短前缀，保留精确 query 的精度同时不空手。
        words = query.split()
        candidates = [query]
        for k in (3, 2):
            if len(words) > k:
...
class ExaProvider:
    def __init__(
        ...
        timeout: int = 45,
        mcporter_config: Path = DEFAULT_MCPORTER_CONFIG,
    ):
        self.runner = runner
        self.timeout = timeout
        self.mcporter_config = mcporter_config

    def search(self, query: str, limit: int = 5) -> list[SearchResult]:
        escaped = query.replace("\\", "\\\\").replace('"', '\\"')
        expression = f'exa.web_search_exa(query: "{escaped}", numResults: {limit})'
        payload = self.runner(
            ["mcporter", "--config", str(self.mcporter_config), "call", expression],
            self.timeout,
        )
...
class JinaReaderProvider:
    ...
    def read_urls(self, urls: list[str]) -> list[SearchResult]:
        ...
        for url in urls:
            ...
            if _jina_failed(payload):
                fallback = _camoufox_read(url)
                results.append(fallback if fallback else parse_jina_text(payload, url))
            else:
